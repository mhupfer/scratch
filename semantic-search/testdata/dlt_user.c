...
/* will be initialized in dlt_start_threads */
...
static DltTraceLoadStat trace_load_stat = {0};
...
static DltReturnValue dlt_initialize_fifo_connection(void)
{
    ...
    int ret;
    // string truncation is okay for too long paths
...
DltReturnValue dlt_init(void)
{
    ...
    dlt_user.trace_load_soft_limit = TRACE_LOAD_CLIENT_SOFT_LIMIT_DEFAULT;
    dlt_user.trace_load_hard_limit = TRACE_LOAD_CLIENT_HARD_LIMIT_DEFAULT;

    if (dlt_user.trace_load_hard_limit
    && (dlt_user.trace_load_soft_limit > dlt_user.trace_load_hard_limit))
    {
        dlt_vnlog(LOG_WARNING, DLT_USER_BUFFER_LENGTH,
                  "Trace soft limit (%u bytes/s) is greater than hard limit. "
                  "Hard limit value: %u bytes/sec is used for soft limit\n",
                  dlt_user.trace_load_soft_limit,
                  dlt_user.trace_load_hard_limit);
        dlt_user.trace_load_soft_limit = dlt_user.trace_load_hard_limit;
    }
...
DltReturnValue dlt_set_filesize_max(unsigned int filesize)
{
    ...
    dlt_vlog(LOG_DEBUG, "%s: Defined filesize_max is [%u]\n", __func__,
...
DltReturnValue dlt_register_injection_callback(DltContext *handle, uint32_t service_id,
                                               int (*dlt_injection_cbk)(uint32_t service_id,
                                                                             void *data,
                                                                             uint32_t length))
{
    return dlt_register_injection_callback_with_id(handle,
                                                   service_id,
                                                   (dlt_injection_callback_id)(void*)dlt_injection_cbk,
                                                   NULL);
}
    ...
    if (dlt_user.dlt_is_file) {
        if (dlt_user_file_reach_max) {
            ...
            if (msg_size > dlt_user.filesize_max) {
                ...
                dlt_vlog(LOG_ERR,
                         "%s: File size (%ld bytes) reached to defined maximum size (%u bytes)\n",
...
#ifdef DLT_TRACE_LOAD_CTRL_ENABLE
    DltUserControlMsgTraceLoadMsg *traceload;
#endif
    ...
    if (fd >= 0) {
        ...
        if (ret) {
            ...
            while (1) {
                ...
                {
                    /* At least, user header, user context, and service id and data_length of injected message is available */
                    if (receiver->bytesRcvd < (int32_t) (sizeof(DltUserHeader) + sizeof(DltUserControlMsgTraceLoadMsg))) {
                        leave_while = 1;
                        break;
                    }

                    traceload = (DltUserControlMsgTraceLoadMsg *)(receiver->buf + sizeof(DltUserControlMsgTraceLoadMsg));

                    pthread_rwlock_wrlock(&trace_load_rw_lock);
                    dlt_user.trace_load_hard_limit = traceload->trace_load_hard_limit;
                    dlt_user.trace_load_soft_limit = traceload->trace_load_soft_limit;
                    pthread_rwlock_unlock(&trace_load_rw_lock);

                    /* keep not read data in buffer */
                    if (dlt_receiver_remove(receiver,
                                            (sizeof(DltUserHeader) + sizeof(DltUserControlMsgTraceLoadMsg))) ==
                                            DLT_RETURN_ERROR)
                        return DLT_RETURN_ERROR;
                }
#endif
